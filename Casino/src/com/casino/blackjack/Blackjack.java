...
public class Blackjack extends JFrame implements ActionListener {
	...
	private JButton dealButton, hitButton, discardButton, standButton,
			resetButton, betButton, doubleButton, quitButton;
	...
	public void setupControl() {
		...
		resetButton = new JButton("Reset Pile");
			...
			if (player.getHand().size() == 5) {
				...
				if (dealer.checkBust()) {
					...
					if (player.getTotal() > dealer.getTotal()) {
						...
						JOptionPane.showMessageDialog(this, "Dealer has "
								+ dealer.getTotal() + ". No money collected.");
					}
				}
			}
		}
	...
	public void bet() {
		...
		if (bet > player.getMoney()) {
			...
			JOptionPane.showMessageDialog(this,
					"There is no playing for free here!");
			return;
		}
		player.placeBet(bet);
		info.update(deck);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (selection.getText().equals("Discard"))
			discard();
		if (selection.getText().equals("Reset Pile"))
			reset();
